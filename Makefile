...
# added for pyyolov2:############################################
CFLAGS+= -fPIC
PYTHON_SHARED_LIB=libpyyolov2.so
PYTHON_SHARED_LIB_PATH=./python/lib/$(PYTHON_SHARED_LIB)
PYTHON_CPP=./src/pyyolov2.cpp 
...
ifeq ($(GPU), 1) 
COMMON+= -DGPU -I/usr/local/cuda/include/ $(PYTHON_INCLUDE)
CFLAGS+= -DGPU
...
all: obj backup results pyyolov2 #$(EXEC)
...
# note to self $^ prints $(OBJS)
pyyolov2: $(OBJS)
	mkdir -p python/lib
	$(CC) $(COMMON) $(CFLAGS) -shared -o $(PYTHON_SHARED_LIB_PATH) $^ $(LDFLAGS)
